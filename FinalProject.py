...
if __name__ == '__main__':
    ...
    fx, fy, cx, cy,_, LUT = ReadCameraModel.ReadCameraModel('FinalProject/Oxford_dataset_reduced/model')


    old_H = np.eye(4)


    folder_dir = 'FinalProject/Oxford_dataset_reduced/images/'
    paths = sorted(os.listdir(folder_dir))
    print(paths)
    for img in range(len(paths)):
        old_frame = cv2.imread(folder_dir+paths[img])
        current_frame = cv2.imread(folder_dir+paths[img+1])
        ...
        new_H = current_H @ old_H
        transformed = np.dot(new_H, origin)
